...
class RefreshWorker(QtCore.QObject):
    """A worker that estimates the user's location by sending signal strength
    data to the server.

    When the response comes back, it emits a location_updated_signal with
    a tuple of the form ``(serverFlag, response)``

    When the response comes back, it immediately tells the server to update
    the user position on the map to that point.
    """

    location_updated_signal = QtCore.Signal(list)  # TODO: Make just list;
...
class PreferencesWindow(QtGui.QDialog):
    ...
    def start_refresh_timer(self):
        ...
        self.refreshTimer.start(Settings.REFRESH_FREQ*1000)
    ...
    def refresh_location(self):
        ...
        if not self.worker_:
            print "A"
            self.worker_ = RefreshWorker()
            print "B"

            self.worker_.location_updated_signal.connect(self.location_slot)

            print "C"
            self.update_signal.connect(self.worker_.get_loc)
            print "D"
            self.worker_.moveToThread(self.bg_thread)
            print "E"
    ...
    def setup_background_thread(self):
        ...
        self.location_worker.moveToThread(worker)
    ...
    # Background actions
    @QtCore.Slot(tuple)
    def location_slot(self, flagResponseTuple):
        '''
        ...
        if flag:
            ...
            sub_menu.addAction(self.other_location_action)
